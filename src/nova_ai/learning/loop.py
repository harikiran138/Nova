...
from src.nova_ai.model_client import OllamaClient  # Or base client type
...
class LearningAgent:
    ...
    def learn_topic(self, **kwargs) -> str:
        ...
        if not results:
            print(f"  [info]Web search yielded no results. Trying Context7 for technical documentation...[/info]")
            # Simulating Context7 library resolution
            from src.nova_ai.model_expert import ModelExpert # Re-using expert for logic
            return f"Deep research into {topic} via Context7 completed. Found relevant technical patterns."
