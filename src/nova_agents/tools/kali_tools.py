...
class KaliTool:
    ...
    def discover_tools(self) -> List[str]:
        """Discover installed tools in the Kali container."""
        if not self.session_active: return []
        common_paths = ["/usr/bin", "/usr/sbin", "/bin", "/sbin"]
        tools = []
        for path in common_paths:
            out = self.run(f"ls {path}")
            if out and "ls:" not in out:
                tools.extend(out.split())
        return list(set(tools))

    # More wrappers
    def wipe(self):
        """Clean up session data."""
        return self.run("rm -rf /workspace/*")
        
    def gobuster(self, url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt") -> str:
        return self.run(f"gobuster dir -u {url} -w {wordlist}")

    def hydra(self, service: str, target: str, user: str, wordlist: str) -> str:
        return self.run(f"hydra -l {user} -P {wordlist} {target} {service}")
